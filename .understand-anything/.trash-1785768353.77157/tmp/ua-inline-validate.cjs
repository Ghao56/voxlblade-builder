#!/usr/bin/env node
const fs = require('fs');
const layersPath = 'C:/Users/Administrator/Downloads/voxlblade-builder/.understand-anything/intermediate/layers.json';
const tourPath = 'C:/Users/Administrator/Downloads/voxlblade-builder/.understand-anything/intermediate/tour.json';
const graphPath = process.argv[2];
const outputPath = process.argv[3];
try {
  const graph = JSON.parse(fs.readFileSync(graphPath, 'utf8'));
  graph.layers = JSON.parse(fs.readFileSync(layersPath, 'utf8'));
  graph.tour = JSON.parse(fs.readFileSync(tourPath, 'utf8'));
