...
const changed = ['src/BaseDamageCalc.svelte','src/BuffList.svelte','src/DamageAnalyzer.svelte','src/DmgTotalTooltip.svelte','src/data/BuffData.ts','src/data/Perkbasedmg.ts','src/lib/constants/buffs.ts','src/lib/constants/perk-base-damage.ts','src/lib/constants/perks.ts','src/lib/constants/storage.ts','src/lib/dmgTypes.ts','src/lib/store.ts','src/lib/types.ts','stats.html'];
const removedIds = new Set(oldGraph.nodes.filter(n => changed.includes(n.filePath)).map(n => n.id));
